...
A 5KΩ potentiometer, an external power supply (in the **9-12Vcc** range) and some buttons complete the device.
...
Before doing this test and if you have your electronics already mounted into a model, **disconnect all the tie rods** to avoid any damage and **disconnect also any ELECTRIC ENGINE or other devices with high current consumption from the on-board system** keeping in mind that **1A is the maximum output current**.  
